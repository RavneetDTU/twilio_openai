...
export function getLaRethaPrompt() {
  ...
  // 2. FIND LA RETHA'S RESTAURANT
  const laRethaConfig = data.restaurants.find(r => r.restaurantId === '5');
  if (!laRethaConfig) {
    throw new Error("La Retha restaurant configuration not found!");
  }

  // 3. EXTRACT SETTINGS
  const { depositAmount, currency, timezone } = laRethaConfig.settings;
  const allHours = laRethaConfig.operatingHours;
...
`;
  ...
  logger.info(`✅ Generating Prompt for La Retha: ${todayName} (Open: ${todaySchedule.open})`);
  ...
  console.log(`✅ Generating Prompt for La Retha: ${todayName} (Open: ${todaySchedule.open})`);


  return `
You are a warm, professional AI assistant for La Retha.
